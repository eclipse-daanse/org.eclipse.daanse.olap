...
public final class LevelContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("Level")
            ...
            .autoEdgeCases()
            .edgeCaseMdx("leaf member",                    "[Geo].[All Geo].[North].Level")
            .edgeCaseMdx("root member (All)",                "[Geo].[All Geo].[North].Parent.Level")
            .edgeCaseMdx("null member",                      "[Geo].[All Geo].[North].Parent.Parent.Level")
            .edgeCaseMdx("hierarchy/dimension reference",    "[Geo].Level")

            // [Geo] is a one-level hierarchy under an All member: F and M sit on the same
            // (only) real level.
            .value("([Geo].[All Geo].[North].Level IS [Geo].[All Geo].[South].Level)", "true")

            .dependsOn("[Geo].[All Geo].[North].Level")
