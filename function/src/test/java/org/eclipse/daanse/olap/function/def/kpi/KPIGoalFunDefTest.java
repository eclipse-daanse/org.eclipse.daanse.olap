...
class KPIGoalFunDefTest {

    private KPIGoalFunDef funDef;

    @BeforeEach
    void setUp() {
        funDef = new KPIGoalFunDef();
    }

    @Test
    @DisplayName("Should have function name KPIGoal")
    void shouldHaveCorrectFunctionName() {
        assertThat(funDef.getFunctionMetaData().operationAtom().name()).isEqualTo("KPIGoal");
    }
    ...
    @Test
    @DisplayName("Should compile call to KPIGoalCalc")
    void shouldCompileToKPIGoalCalc() {
        ...
        assertThat(result).isInstanceOf(KPIGoalCalc.class);
    }
}
