...
/**
 * {@code MDSCHEMA_DIMENSIONS}, as a whiteboard service.
 * <p>
 * The dimensions of each cube, their type and cardinality.
 * <p>
...
@Component(service = RowsetProvider.class, scope = ServiceScope.PROTOTYPE, property = RowsetProvider.PROPERTY_REQUEST_TYPE
        + "=MDSCHEMA_DIMENSIONS")
public class MdschemaDimensionsProvider implements RowsetProvider<ContextListSupplyer> {

    @Override
    public List<EObject> rows(RowsetScope<ContextListSupplyer> scope) {
        return new MdSchemaDiscover(scope.backend(), List.of()).dimensions(scope.restrictions(), scope.caller());
    }
}
