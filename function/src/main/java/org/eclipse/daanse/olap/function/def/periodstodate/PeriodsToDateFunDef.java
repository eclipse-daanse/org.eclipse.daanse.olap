...
public class PeriodsToDateFunDef extends AbstractFunctionDefinition {
    ...
    public Type getResultType(Validator validator, Expression[] args) {
        ...
        if (args.length >= 2) {
            ...
            // A NULL literal is a NullType, not a MemberType, and the cast used to end
            // validation with a ClassCastException. Say what is wrong instead.
            if (!(args[1].getType() instanceof MemberType)) {
                throw Util.newError("Type mismatch: the second argument of "
                        + getFunctionMetaData().operationAtom().name() + " must be a member, and "
                        + args[1].getType().getClass().getSimpleName() + " is not one");
            }
