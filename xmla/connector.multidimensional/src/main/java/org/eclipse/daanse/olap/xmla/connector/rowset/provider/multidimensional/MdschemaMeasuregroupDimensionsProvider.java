...
/**
 * {@code MDSCHEMA_MEASUREGROUP_DIMENSIONS}, as a whiteboard service.
 * <p>
 * Which dimensions a measure group is related to, and how - the cube's
 * dimensionality, one row per pair.
 * <p>
...
public class MdschemaMeasuregroupDimensionsProvider implements RowsetProvider<ContextListSupplyer> {

    @Override
    public List<EObject> rows(RowsetScope<ContextListSupplyer> scope) {
        return new MdSchemaDiscover(scope.backend(), List.of()).measureGroupDimensions(scope.restrictions(),
                scope.caller());
    }
}
