...
import org.eclipse.daanse.olap.calc.base.type.hierarchy.HierarchyCurrentMemberFixedCalc;
...
public class OpeningClosingPeriodFunDef extends AbstractFunctionDefinition {
    ...
    public Calc<?> compileCall( ResolvedFunCall call, ExpressionCompiler compiler) {
        ...
        case 1:
            ...
            // OpeningClosingPeriodFunDef instance with a distinct FunctionMetaData. A genuine
            // one-arg Member call must not be silently reinterpreted as a Level with an
            // implicit default-Time member, or the member argument passed in is discarded
            // entirely and a completely unrelated dimension's current member is used instead.
            // The argument's own type decides, as in Mondrian: only a member expression takes
            // the member path. A hierarchy or dimension argument also binds to the (Member)
            // overload (through the implicit CurrentMember conversion), but it names a level
            // of the cube's Time hierarchy, so it takes the level path and reports "no time
            // dimension" on a cube without one instead of quietly answering something else.
            if (getFunctionMetaData().parameters()[0].dataType() == DataType.MEMBER
                    && call.getArg(0).getType() instanceof MemberType) {
                levelCalc = null;
