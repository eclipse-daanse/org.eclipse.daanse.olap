...
public class KPIValueCalc  extends AbstractProfilingNestedMemberCalc {

    protected KPIValueCalc(Type type, StringCalc kpiCalc) {
        super(type, kpiCalc);
    }
    ...
    protected Member evaluateInternal(Evaluator evaluator) {
        ...
        if (oKpi.isPresent()) {
            String value = oKpi.get().getValue();
            Member member = FunUtil.parseMember(evaluator, value, null);
            return member;
        } else {
            throw FunUtil.newEvalException(
                    new OlapRuntimeException("Value is absent for " + kpi));
        }
    }
