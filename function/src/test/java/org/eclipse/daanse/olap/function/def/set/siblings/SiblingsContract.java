 ...
 * [Geo].[All Geo].[North].Parent.Parent} on this flat, {@code hasAll=true} fixture) has no siblings at
...
public final class SiblingsContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("Siblings")
            ...
            .autoEdgeCases()
            .edgeCaseMdx("ordinary member",  "[Geo].[All Geo].[North].Siblings")
            .edgeCaseMdx("root member (All)", "[Geo].[All Geo].[North].Parent.Siblings")
            .edgeCaseMdx("null member",       "[Geo].[All Geo].[North].Parent.Parent.Siblings")

            // [Geo] is flat: F and M are both children of the All member, so Siblings of
            // either (which includes the member itself) is the full two-member set.
            .value("Count([Geo].[All Geo].[North].Siblings)", "2")
            ...
            .value("Count([Geo].[All Geo].[North].Parent.Siblings)", "1")
            // The null member has no siblings at all, not even itself.
            .value("Count([Geo].[All Geo].[North].Parent.Parent.Siblings)", "0")

            .dependsOn("[Geo].[All Geo].[North].Siblings")

            .resultStyle("[Geo].[All Geo].[North].Siblings", ResultStyle.MUTABLE_LIST, ResultStyle.MUTABLE_LIST)
            .resultStyle("[Geo].[All Geo].[North].Siblings", ResultStyle.ITERABLE, ResultStyle.MUTABLE_LIST)
            .independentMutableList("[Geo].[All Geo].[North].Siblings")
