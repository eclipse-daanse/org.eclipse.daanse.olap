...
public final class LagContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("Lag")
            ...
            .autoEdgeCases()
            .edgeCaseMdx("zero lag",                 "[Geo].[All Geo].[North].Lag(0)")
            .edgeCaseMdx("positive lag within range", "[Geo].[All Geo].[South].Lag(1)")
            .edgeCaseMdx("negative lag (= lead)",     "[Geo].[All Geo].[North].Lag(-1)")
            .edgeCaseMdx("lag out of range",          "[Geo].[All Geo].[North].Lag(1)")
            // Fixed: used to NPE unboxing a NULL amount before the null check existed.
            .edgeCaseMdx("NULL amount (fixed)",       "[Geo].[All Geo].[North].Lag(NULL)")
            // The explicit Integer.MIN_VALUE overflow guard in LeadLagCalc.
            .edgeCaseMdx("Integer.MIN_VALUE amount",  "[Geo].[All Geo].[North].Lag(-2147483648)")
