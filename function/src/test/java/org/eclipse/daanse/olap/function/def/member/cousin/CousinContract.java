 ...
 *
 * <p>{@code [Geo]} is flat (only one real level beneath {@code All}), so there is no
...
public final class CousinContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("Cousin")
            ...
            .autoEdgeCases()
            .edgeCaseMdx("self cousin (ancestor is own parent)", "Cousin([Geo].[All Geo].[North], [Geo].[All Geo].[North].Parent)")
            .edgeCaseMdx("ancestor NULL",                 "Cousin([Geo].[All Geo].[North], [Geo].[All Geo].[North].Parent.Parent)")
            .edgeCaseMdx("member shallower than ancestor", "Cousin([Geo].[All Geo].[North].Parent, [Geo].[All Geo].[North])")
            ...
            .edgeCaseMdx("hierarchy mismatch (documented gap)",
                    "Cousin([Geo].[All Geo].[North], [Measures].[Amount])")

            .value("(Cousin([Geo].[All Geo].[North], [Geo].[All Geo].[North].Parent) IS [Geo].[All Geo].[North])", "true")
            .value("(Cousin([Geo].[All Geo].[North], [Geo].[All Geo].[North].Parent.Parent) IS [Geo].[All Geo].[North].Parent.Parent)", "true")
            .value("(Cousin([Geo].[All Geo].[North].Parent, [Geo].[All Geo].[North]) IS [Geo].[All Geo].[North].Parent.Parent)", "true")

            .dependsOn("Cousin([Geo].[All Geo].[North], [Geo].[All Geo].[North].Parent)")

            // A scalar call has no set to shape. That it answers VALUE was asserted in
            // prose here and never checked, so it is a case now.
            .scalarResultStyle("(Cousin([Geo].[All Geo].[North], [Geo].[All Geo].[North].Parent) IS [Geo].[All Geo].[North])")
            .build();
}
