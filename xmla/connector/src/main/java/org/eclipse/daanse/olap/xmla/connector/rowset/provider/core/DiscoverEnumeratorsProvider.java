...
/**
 * {@code DISCOVER_ENUMERATORS}, as a whiteboard service.
 * <p>
 * The named vocabularies other Discover answers refer to, and the values each
 * allows. Read out of the Ecore model rather than written here, because XMLA
 * has the provider publish its own numbers.
 * <p>
...
public class DiscoverEnumeratorsProvider implements RowsetProvider<ContextListSupplyer> {

    @Override
    public List<EObject> rows(RowsetScope<ContextListSupplyer> scope) {
        return new OtherDiscover(scope.backend()).enumerators();
    }
}
