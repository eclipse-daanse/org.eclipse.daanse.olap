...
public final class LevelNumberContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("Level_Number")
            ...
            .autoEdgeCases()
            .edgeCaseMdx("leaf member",                 "[Geo].[All Geo].[North].Level_Number")
            .edgeCaseMdx("root member (All)",             "[Geo].[All Geo].[North].Parent.Level_Number")
            .edgeCaseMdx("null member",                   "[Geo].[All Geo].[North].Parent.Parent.Level_Number")
            .edgeCaseMdx("hierarchy/dimension reference", "[Geo].Level_Number")

            // [Geo] is a one-level hierarchy under an All member: the All member sits at
            // depth 0, and its (only) real level of children — F, M — sits at depth 1.
            .value("[Geo].[All Geo].[North].Level_Number", "1")
            .value("[Geo].[All Geo].[South].Level_Number", "1")
            .value("[Geo].[All Geo].[North].Parent.Level_Number", "0")

            .scalarDependsOn("[Geo].[All Geo].[North].Level_Number")

            // A scalar call has no set to shape. That it answers VALUE was asserted in
            // prose here and never checked, so it is a case now.
            .scalarResultStyle("[Geo].[All Geo].[North].Level_Number")
