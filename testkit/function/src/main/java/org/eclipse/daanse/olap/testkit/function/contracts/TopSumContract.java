 ...
 * {@code [Measures].[Unit Sales]} over {@code [Gender].Members} means a target well beyond
...
public final class TopSumContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("TopSum")
            ...
            .autoEdgeCases()
            .edgeCaseMdx("empty set",              "TopSum({}, 50, [Measures].[Unit Sales])")
            .edgeCaseMdx("target zero",             "TopSum([Gender].Members, 0, [Measures].[Unit Sales])")
            .edgeCaseMdx("target -1",               "TopSum([Gender].Members, -1, [Measures].[Unit Sales])")
            .edgeCaseMdx("target negative",         "TopSum([Gender].Members, -10, [Measures].[Unit Sales])")
            .edgeCaseMdx("target fractional",       "TopSum([Gender].Members, 0.5, [Measures].[Unit Sales])")
            .edgeCaseMdx("target beyond total",     "TopSum([Gender].Members, 1000000, [Measures].[Unit Sales])")
            .edgeCaseMdx("target 1E308",            "TopSum([Gender].Members, 1E308, [Measures].[Unit Sales])")
            .edgeCaseMdx("target NULL",             "TopSum([Gender].Members, NULL, [Measures].[Unit Sales])")
            .edgeCaseMdx("value expression zero",       "TopSum([Gender].Members, 50, 0)")
            .edgeCaseMdx("value expression negative",   "TopSum([Gender].Members, 50, -1)")
            .edgeCaseMdx("value expression fractional", "TopSum([Gender].Members, 50, 0.5)")
            .edgeCaseMdx("value expression 1E308",      "TopSum([Gender].Members, 50, 1E308)")
            .edgeCaseMdx("value expression NULL",   "TopSum([Gender].Members, 50, NULL)")

            .value("Count(TopSum([Gender].Members, 0, [Measures].[Unit Sales]))",    "0")
            .value("Count(TopSum([Gender].Members, -1, [Measures].[Unit Sales]))",   "0")
            ...
            .independentMutableList("TopSum([Gender].Members, 50, [Measures].[Unit Sales])")
            .build();
}
