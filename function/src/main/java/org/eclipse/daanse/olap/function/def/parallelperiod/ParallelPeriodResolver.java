...
public class ParallelPeriodResolver extends AbstractFunctionDefinitionMultiResolver {
    ...
    // The parameter descriptions, text keys and captions come from main; the split into one
    // declaration per arity stays, see the constructor comment below for why.
    ...
    private static FunctionMetaData functionMetaDataWithLevel = new FunctionMetaDataR(atom, DESCRIPTION,
            ...
            .withTextKey("ParallelPeriod.Level").caption("ParallelPeriod Function");
    private static FunctionMetaData functionMetaDataWithLevelNumeric = new FunctionMetaDataR(atom, DESCRIPTION,
            DataType.MEMBER, new FunctionParameterR[] { FunctionParameterR.param(DataType.LEVEL).describedAs("Level"),
                    FunctionParameterR.param(DataType.NUMERIC).describedAs("Numeric") })
            .interfaceName(FunctionInterface.DATETIME)
            .withTextKey("ParallelPeriod.Level.Numeric").caption("ParallelPeriod Function");
    private static FunctionMetaData functionMetaDataWithLevelNumericMember = new FunctionMetaDataR(atom, DESCRIPTION,
            DataType.MEMBER, new FunctionParameterR[] { FunctionParameterR.param(DataType.LEVEL).describedAs("Level"),
                    FunctionParameterR.param(DataType.NUMERIC).describedAs("Numeric"),
                    FunctionParameterR.param(DataType.MEMBER).describedAs("Member") })
            .interfaceName(FunctionInterface.DATETIME)
            .withTextKey("ParallelPeriod.Level.Numeric.Member").caption("ParallelPeriod Function");
