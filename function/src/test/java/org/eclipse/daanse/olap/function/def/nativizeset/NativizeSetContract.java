 ...
 * {@code parent().evaluate(evaluator)} — so {@code NativizeSet([Geo].Members)} evaluates
 * identically to {@code [Geo].Members} alone.
 */
public final class NativizeSetContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("NativizeSet")
            ...
            .edgeCaseMdx("simple set",              "NativizeSet([Geo].Members)")
            .edgeCaseMdx("member operand (lenient)", "NativizeSet([Geo].[All Geo].[North])")
            .edgeCaseMdx("crossjoined set",          "NativizeSet([Geo].Members * [Measures].[Amount])")

            .value("Count(NativizeSet([Geo].Members))", "8")
            .value("SetToStr(NativizeSet({[Geo].[All Geo].[North]}))", "{[Geo].[All Geo].[North]}")

            .dependsOn("NativizeSet([Geo].Members)")

            .resultStyle("NativizeSet([Geo].Members)", ResultStyle.MUTABLE_LIST, ResultStyle.MUTABLE_LIST)
            .resultStyle("NativizeSet([Geo].Members)", ResultStyle.ITERABLE, ResultStyle.MUTABLE_LIST)
            .independentMutableList("NativizeSet([Geo].Members)")
