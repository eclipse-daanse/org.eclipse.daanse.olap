...
public class KPIStatusCalc  extends AbstractProfilingNestedMemberCalc {

    protected KPIStatusCalc(Type type, StringCalc kpiCalc) {
        super(type, kpiCalc);
    }
    ...
    protected Member evaluateInternal(Evaluator evaluator) {
        ...
        if (oKpi.isPresent()) {
            String status = oKpi.get().getStatus();
            Member member = FunUtil.parseMember(evaluator, status, null);
            return member;
        } else {
            throw FunUtil.newEvalException(
                    new OlapRuntimeException("Status is absent for " + kpi));
        }
    }
