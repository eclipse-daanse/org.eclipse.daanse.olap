 ...
 * shepherding the creation of a Result by running the actual execution
 ...
 * @author LBoudreau
 */
public class ResultShepherdImpl implements ResultShepherd {
    ...
    private final Timer timer =
    		new Timer("daanse.olap.ResultShepherdImpl#timer", true);

    public ResultShepherdImpl(final long connectionShepherdThreadPollingInterval, TimeUnit connectionShepherdThreadPollingIntervalUnit, final int connectionShepherdNbThreads) {

        final int maximumPoolSize = connectionShepherdNbThreads;
        executor =
            Util.getExecutorService(
                ...
                maximumPoolSize,
                1000000,
                "daanse.olap.ResultShepherdImpl$executor",
                new RejectedExecutionHandler() {
                        ...
                        ThreadPoolExecutor executor)
                    {
                        throw new OlapRuntimeException(MessageFormat.format(queryLimitReached,
                            maximumPoolSize,
                            "connectionShepherdNbThreads"));
                    }
                });

        long period = connectionShepherdThreadPollingIntervalUnit.toMillis(connectionShepherdThreadPollingInterval);
        timer.schedule(
            new TimerTask() {
                @Override
				public void run() {
