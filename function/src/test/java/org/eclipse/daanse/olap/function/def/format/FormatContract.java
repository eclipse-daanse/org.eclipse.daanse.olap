 ...
 * argument like {@code [Measures].[Amount]} is coerced to a scalar via an implicit {@code
 ...
 * the cube. A dynamic reference like {@code [Geo].CurrentMember.Name} has no such fixed
...
public final class FormatContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("Format")
            ...
            .edgeCaseMdx("member value",                         "Format([Measures].[Amount], \"#,##0\")")
            .edgeCaseMdx("date value",                           "Format(Now(), \"yyyy\")")
            .edgeCaseMdx("variable (non-literal) format string", "Format(1234, [Geo].CurrentMember.Name)")
            ...
            // Reuses the fact established by SumContract: Unit Sales summed over [Geo] is
            // 266,773; a Member value formats its current cell value, same as a plain scalar
            // reference to it would.
            .value("Format([Measures].[Amount], \"#,##0\")", "150")

            .scalarDoesNotDependOn("Format([Measures].[Amount], \"#,##0\")", "[Measures]")
            .scalarDependsOn("Format(1234, [Geo].CurrentMember.Name)", "[Geo]")

            // A scalar call has no set to shape. That it answers VALUE was asserted in
            // prose here and never checked, so it is a case now.
            .scalarResultStyle("Format(1234, \"#,##0\")")
            .build();
}
