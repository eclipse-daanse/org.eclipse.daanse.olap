...
public class KPIWeightCalc  extends AbstractProfilingNestedMemberCalc {

    protected KPIWeightCalc(Type type, StringCalc kpiCalc) {
        super(type, kpiCalc);
    }
    ...
    protected Member evaluateInternal(Evaluator evaluator) {
        ...
        if (oKpi.isPresent()) {
            String weight = oKpi.get().getWeight();
            Member member = FunUtil.parseMember(evaluator, weight, null);
            return member;
        } else {
            throw FunUtil.newEvalException(
                    new OlapRuntimeException("Weight is absent for " + kpi));
        }
    }
