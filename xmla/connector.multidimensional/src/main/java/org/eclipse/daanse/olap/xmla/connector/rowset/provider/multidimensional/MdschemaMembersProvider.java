...
/**
 * {@code MDSCHEMA_MEMBERS}, as a whiteboard service.
 * <p>
 * The members of a hierarchy, restricted by level, by parent, or by the TREE_OP
 * bitmask that asks for a member's relatives.
 * <p>
...
@Component(service = RowsetProvider.class, scope = ServiceScope.PROTOTYPE, property = RowsetProvider.PROPERTY_REQUEST_TYPE
        + "=MDSCHEMA_MEMBERS")
public class MdschemaMembersProvider implements RowsetProvider<ContextListSupplyer> {

    @Override
    public List<EObject> rows(RowsetScope<ContextListSupplyer> scope) {
        return new MdSchemaDiscover(scope.backend(), List.of()).members(scope.restrictions(), scope.caller());
    }
}
