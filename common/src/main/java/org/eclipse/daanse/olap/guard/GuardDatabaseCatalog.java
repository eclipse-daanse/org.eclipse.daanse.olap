...
public class GuardDatabaseCatalog implements DatabaseCatalog{

    private List<DatabaseSchema> databaseSchemas;
    private String name;


    public GuardDatabaseCatalog(String name, List<org.eclipse.daanse.olap.api.element.db.DatabaseSchema> databaseSchemas) {
        this.name = name;
        if (databaseSchemas != null) {
            this.databaseSchemas = databaseSchemas.stream().map(ds -> (DatabaseSchema)new GuardDatabaseSchema(ds)).toList();
        } else {
