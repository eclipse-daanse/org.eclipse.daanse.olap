...
public class DatePartFunDef  extends AbstractFunctionDefinition {
    ...
    public Calc<?> compileCall(ResolvedFunCall call, ExpressionCompiler compiler) {
        ...
        IntegerCalc firstDayOfWeek = new ConstantIntegerCalc(new DecimalType(Integer.MAX_VALUE, 0), Calendar.SUNDAY);
        IntegerCalc firstWeekOfYear = new ConstantIntegerCalc(new DecimalType(Integer.MAX_VALUE, 0), 1);
        if (call.getArgCount() >= 3) {
            firstDayOfWeek = compiler.compileInteger(call.getArg(2));
        }
        if (call.getArgCount() >= 4) {
            firstWeekOfYear = compiler.compileInteger(call.getArg(3));
        }
