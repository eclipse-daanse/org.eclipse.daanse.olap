...
public class MeasuryEmiter {
    ...
    public void emitMeasures(TSchema schema, TEntityType measuresType, Cube cube, EmitContext ctx) {
        for (Member member : cube.getMeasures()) {                       // MEA-01
            MeasureKind kind =
                    member instanceof StoredMeasure   ? MeasureKind.STORED
                  : member.isCalculated()             ? MeasureKind.CALCULATED
                  : MeasureKind.STORED;
            ...
            for (String assignment : path.split(";")) {
                ...
                for (String segment : assignment.split("[\\\\/]")) {
                    ...
                    if (folder == null) {
                        ...
                        (parent == null ? root.getDisplayFolder()
                                        : parent.getDisplayFolder()).add(folder);
                ...
                ctx.referenceProperty(measuresType.getName(), propName, "bi:DisplayFolder");
    ...
    private void emitMeasureProperty(TEntityType owner, Member member, MeasureKind kind, EmitContext ctx) {
        ...
        Optional<String> aggName = (member instanceof StoredMeasure sm)
                ? Optional.of(sm.getAggregateFunction())
                : Optional.empty();

        Optional<TypeMapper.EdmType> source = sourceTypeOf(member, ctx);  // s. u.
        TypeMapper.EdmType type = ctx.types()
                .measureType(aggName.orElse("None"), source);
        ...
        boolean simple = kind == MeasureKind.STORED
                && aggName.map(SIMPLE_AGGREGATORS::contains).orElse(false);
    ...
    void emitDefaultMeasure(TEntityType measuresType, Cube cube, EmitContext ctx) {
        Hierarchy measuresHierarchy = cube.getDimensions().stream()
                .filter(Dimension::isMeasures).findFirst()
        ...
        ref.setName(propName);
        ctx.referenceProperty(measuresType.getName(), propName, "bi:DefaultMeasure");
        dm.setMemberRef(ref);
    ...
    private static final Set<String> SIMPLE_AGGREGATORS =
            Set.of("sum", "count", "avg", "min", "max", "distinct-count");
    ...
    enum MeasureKind { STORED, CALCULATED }
}
