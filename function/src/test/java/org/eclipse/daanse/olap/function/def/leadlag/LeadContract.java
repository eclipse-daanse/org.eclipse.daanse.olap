...
public final class LeadContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("Lead")
            ...
            .autoEdgeCases()
            .edgeCaseMdx("zero lead",                 "[Geo].[All Geo].[North].Lead(0)")
            .edgeCaseMdx("positive lead within range", "[Geo].[All Geo].[North].Lead(1)")
            .edgeCaseMdx("negative lead (= lag)",      "[Geo].[All Geo].[South].Lead(-1)")
            .edgeCaseMdx("lead out of range",          "[Geo].[All Geo].[South].Lead(1)")
            // Fixed (see LagContract): used to NPE unboxing a NULL amount before the null
            // check existed in the shared LeadLagCalc.
            .edgeCaseMdx("NULL amount (fixed)",        "[Geo].[All Geo].[North].Lead(NULL)")
            .edgeCaseMdx("Integer.MIN_VALUE amount",   "[Geo].[All Geo].[North].Lead(-2147483648)")
            ...
            .valueKnownDefect("([Geo].[All Geo].[North].Lead(0) IS [Geo].[All Geo].[North])", "true",
            ...
            .valueKnownDefect("([Geo].[All Geo].[North].Lead(1) IS [Geo].[All Geo].[South])", "true",
                            "a method call that takes an argument does not resolve: the"
                            + " argument is lost and the receiver is typed as a numeric"
                            + " expression. The no-argument forms work. The defect is in the MDX"
                            + " parser, org.eclipse.daanse.mdx, not here")
            .valueKnownDefect("([Geo].[All Geo].[South].Lead(-1) IS [Geo].[All Geo].[North])", "true",
                            "a method call that takes an argument does not resolve: the"
                            + " argument is lost and the receiver is typed as a numeric"
                            + " expression. The no-argument forms work. The defect is in the MDX"
                            + " parser, org.eclipse.daanse.mdx, not here")

            .dependsOnKnownDefect("[Geo].[All Geo].[North].Lead(1)",
                            "a method call that takes an argument does not resolve: the"
                            + " argument is lost and the receiver is typed as a numeric"
