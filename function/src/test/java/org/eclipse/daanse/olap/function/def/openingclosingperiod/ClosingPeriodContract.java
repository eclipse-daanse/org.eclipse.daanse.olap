...
public final class ClosingPeriodContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("ClosingPeriod")
            ...
            .edgeCaseMdx("member only",         "ClosingPeriod([Geo].[All Geo].[North].Parent)")
            .edgeCaseMdx("member with no descendant (null result)", "ClosingPeriod([Geo].[All Geo].[North])")
            .edgeCaseMdx("level and member",    "ClosingPeriod([Geo].[All Geo].[North].Level, [Geo].[All Geo].[North].Parent)")

            // [Geo] is flat (hasAll=true, 2 levels: All and Gender). Children of the All
            ...
            .value("(ClosingPeriod([Geo].[All Geo].[North].Level, [Geo].[All Geo].[North].Parent) IS [Geo].[All Geo].[South])", "true")
            .value("(ClosingPeriod([Geo].[All Geo].[North].Parent) IS [Geo].[All Geo].[South])", "true")
            // F is already the deepest real level: there is no level below it to descend to.
            .value("(ClosingPeriod([Geo].[All Geo].[North]) IS [Geo].[All Geo].[North].Parent.Parent)", "false")

            .dependsOn("ClosingPeriod([Geo].[All Geo].[North].Level, [Geo].[All Geo].[North].Parent)")

            // A scalar call has no set to shape. That it answers VALUE was asserted in
            // prose here and never checked, so it is a case now.
            .scalarResultStyle("(ClosingPeriod([Geo].[All Geo].[North].Level, [Geo].[All Geo].[North].Parent) IS [Geo].[All Geo].[South])")
            .build();
}
