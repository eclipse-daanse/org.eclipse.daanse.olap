 ...
 * value assertions below cover both shapes, grounded in the same {@code [Geo].[All Geo].[North]} /
 * {@code [Measures].[Amount]} facts {@link AsContract} and {@link CrossjoinContract} use.
 */
public final class SetToStrContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("SetToStr")
            ...
            .edgeCaseMdx("member set",              "SetToStr([Geo].Members)")
            .edgeCaseMdx("tuple set",                "SetToStr({([Geo].[All Geo].[North], [Measures].[Amount])})")
            .edgeCaseMdx("member operand (lenient)", "SetToStr([Geo].[All Geo].[North])")

            .value("SetToStr({})", "{}")
            .value("SetToStr({[Geo].[All Geo].[North]})", "{[Geo].[All Geo].[North]}")
            .value("SetToStr({([Geo].[All Geo].[North], [Measures].[Amount])})",
                   "{([Geo].[All Geo].[North], [Measures].[Amount])}")

            .scalarDependsOn("SetToStr([Geo].Members)")

            // A scalar call has no set to shape. That it answers VALUE was asserted in
            // prose here and never checked, so it is a case now.
            .scalarResultStyle("SetToStr({})")
