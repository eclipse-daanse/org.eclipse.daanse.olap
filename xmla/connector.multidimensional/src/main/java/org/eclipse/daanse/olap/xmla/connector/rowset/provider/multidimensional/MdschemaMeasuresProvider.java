...
/**
 * {@code MDSCHEMA_MEASURES}, as a whiteboard service.
 * <p>
 * The measures of each cube, with their aggregator, data type and display
 * folder.
 * <p>
...
@Component(service = RowsetProvider.class, scope = ServiceScope.PROTOTYPE, property = RowsetProvider.PROPERTY_REQUEST_TYPE
        + "=MDSCHEMA_MEASURES")
public class MdschemaMeasuresProvider implements RowsetProvider<ContextListSupplyer> {

    @Override
    public List<EObject> rows(RowsetScope<ContextListSupplyer> scope) {
        return new MdSchemaDiscover(scope.backend(), List.of()).measures(scope.restrictions(), scope.caller());
    }
}
