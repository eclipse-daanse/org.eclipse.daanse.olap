...
public final class SubsetContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("Subset")
            ...
            .autoEdgeCases()
            .edgeCaseMdx("start 0, no count",     "Subset([Geo].Members, 0)")
            .edgeCaseMdx("start beyond end",       "Subset([Geo].Members, 100)")
            .edgeCaseMdx("start negative",         "Subset([Geo].Members, -1)")
            .edgeCaseMdx("start MIN_VALUE",        "Subset([Geo].Members, -2147483648)")
            .edgeCaseMdx("start NULL",             "Subset([Geo].Members, NULL)")
            .edgeCaseMdx("count beyond end",       "Subset([Geo].Members, 0, 100)")
            .edgeCaseMdx("count NULL",             "Subset([Geo].Members, 0, NULL)")
            ...
            .edgeCaseMdx("count MAX_VALUE (documented overflow gap)",
                    "Subset([Geo].Members, 1, 2147483647)")

            .value("Count(Subset([Geo].Members, 0))", "8")
            .value("Count(Subset([Geo].Members, 1))", "7")
            .valueKnownDefect("Subset([Geo].Members, 1).Item(0).Name", "F",
            ...
            .value("Count(Subset([Geo].Members, 0, 1))", "1")
            .valueKnownDefect("Subset([Geo].Members, 0, 1).Item(0).Name", "All Gender",
                            "a method call that takes an argument does not resolve: the"
                            + " argument is lost and the receiver is typed as a numeric"
                            + " expression. The no-argument forms work. The defect is in the MDX"
                            + " parser, org.eclipse.daanse.mdx, not here")
            .value("Count(Subset([Geo].Members, 5))", "3")
            .value("Count(Subset([Geo].Members, NULL))", "0")

            .dependsOn("Subset([Geo].Members, 0, 1)")

            .resultStyle("Subset([Geo].Members, 0, 1)", ResultStyle.MUTABLE_LIST, ResultStyle.MUTABLE_LIST)
            .resultStyle("Subset([Geo].Members, 0, 1)", ResultStyle.ITERABLE, ResultStyle.MUTABLE_LIST)
            .independentMutableList("Subset([Geo].Members, 0, 1)")
