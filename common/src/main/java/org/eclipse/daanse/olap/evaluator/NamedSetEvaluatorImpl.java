 ...
 */
public class NamedSetEvaluatorImpl implements Evaluator.NamedSetEvaluator, TupleList.PositionCallback {

    private static final Logger LOGGER = LoggerFactory.getLogger(NamedSetEvaluatorImpl.class);
  private final EvaluatorRoot rrer;
  ...
  /**
   * Creates a NamedSetEvaluatorImpl.
   *
   * @param rrer
   ...
   *          Named set
   */
  public NamedSetEvaluatorImpl( EvaluatorRoot rrer, NamedSet namedSet ) {
    this.rrer = rrer;
...
public TupleIterable evaluateTupleIterable( Evaluator evaluator ) {
  ...
  private void ensureList( Evaluator evaluator ) {
    if ( list != null ) {
      if ( list == DUMMY_LIST ) {
        ...
        if ( iterationLimit > 0 && recursionCount > iterationLimit ) {
          throw rrer.slicerEvaluator().newEvalException( null,
    ...
    if ( LOGGER.isDebugEnabled() ) {
      LOGGER.debug( new StringBuilder("Named set ")
    ...
    try {
      ...
      TupleIterable iterable = (TupleIterable) rrer.evaluateExpression( calc, rrer.slicerEvaluator(), evaluator );
      ...
      if ( LOGGER.isDebugEnabled() ) {
        LOGGER.debug( generateDebugMessage( calc, rawList ) );
      }
