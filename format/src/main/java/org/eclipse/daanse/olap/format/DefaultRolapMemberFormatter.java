 ...
 *
 * Must be used for {@link MemberBase} only.
 */
public class DefaultRolapMemberFormatter implements MemberFormatter {
     ...
     *   for {@link MemberBase} objects formatting.
     *
     ...
     *   in org.eclipse.daanse.rolap.element.RolapLevel only,
     *   and IS only used for RolapMemberBase.
     * 
     */
    @Override
    public String format(Member member) {
        if (member instanceof MemberBase rolapMember) {
    ...
    private String doFormatMember(MemberBase rolapMember) {
