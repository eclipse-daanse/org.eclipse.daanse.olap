...
public abstract class AbstractFunctionDefinition implements FunctionDefinition {
	...
	public Type getResultType(Validator validator, Expression[] args) {
		...
		// An OlapRuntimeException, not an IllegalArgumentException: a call whose type cannot
		// be deduced is a query the engine refuses, not a programming error in the caller.
		// The distinction is what tells a diagnosed rejection from an accident, and the
		// wrong type made every such refusal look like the latter.
		throw new OlapRuntimeException(new StringBuilder("Cannot deduce type of call to function '")
