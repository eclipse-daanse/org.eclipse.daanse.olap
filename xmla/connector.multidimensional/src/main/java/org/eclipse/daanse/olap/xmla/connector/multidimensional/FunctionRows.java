 ...
 * <p>
 * The rows are model EObjects and the nested parameters are containment
 * children, so they reach the wire with the rest of the row.
 */
public final class FunctionRows {
    ...
    private static ParameterInfo parameterInfo(FunctionParameter parameter, ResolvedFunctionTexts texts) {
        ...
        // [MS-SSAS] 37.0 (2026-05-25) changed SKIPPABLE from xsd:int to xsd:boolean;
        // the value was always a yes or no and only the schema said otherwise.
        info.setSkippable(parameter.skippable());
        return info;
    }
