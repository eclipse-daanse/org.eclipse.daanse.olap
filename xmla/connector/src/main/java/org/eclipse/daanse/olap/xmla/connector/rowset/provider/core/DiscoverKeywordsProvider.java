...
/**
 * {@code DISCOVER_KEYWORDS}, as a whiteboard service.
 * <p>
 * The reserved words of the query language, so a client can quote identifiers
 * correctly.
 * <p>
...
@Component(service = RowsetProvider.class, scope = ServiceScope.PROTOTYPE, property = RowsetProvider.PROPERTY_REQUEST_TYPE
        + "=DISCOVER_KEYWORDS")
public class DiscoverKeywordsProvider implements RowsetProvider<ContextListSupplyer> {

    @Override
    public List<EObject> rows(RowsetScope<ContextListSupplyer> scope) {
        return new OtherDiscover(scope.backend()).keywords();
    }
}
