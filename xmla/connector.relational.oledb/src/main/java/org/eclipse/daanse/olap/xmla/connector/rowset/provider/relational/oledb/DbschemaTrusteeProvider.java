...
/**
 * {@code DBSCHEMA_TRUSTEE}, as a whiteboard service — <strong>answering no rows
 * yet</strong>.
 * <p>
 * The trustees defined on the data store. An OLE DB schema rowset that is <em>not</em> in Appendix B — a later addition,
 * documented only through {@code System.Data.OleDb.OleDbSchemaGuid.Trustee}. It
 * is offered so that a consumer speaking OLE DB over XMLA finds it under the
 * name and GUID it knows.
 * <p>
 * <strong>To fill it</strong>, the catalog needs a notion of trustees — the accounts and groups a data store knows. The mapping model
...
@Component(service = RowsetProvider.class, scope = ServiceScope.PROTOTYPE, property = RowsetProvider.PROPERTY_REQUEST_TYPE
        + "=DBSCHEMA_TRUSTEE")
public class DbschemaTrusteeProvider implements RowsetProvider<ContextListSupplyer> {
