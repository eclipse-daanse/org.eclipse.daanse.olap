...
public final class OrderContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("Order")
            ...
            .edgeCaseMdx("default ASC",             "Order([Geo].Members, [Geo].CurrentMember.Name)")
            .edgeCaseMdx("explicit ASC",            "Order([Geo].Members, [Geo].CurrentMember.Name, ASC)")
            .edgeCaseMdx("explicit DESC",           "Order([Geo].Members, [Geo].CurrentMember.Name, DESC)")
            .edgeCaseMdx("break hierarchy (BASC)",  "Order([Geo].Members, [Geo].CurrentMember.Name, BASC)")
            .edgeCaseMdx("two sort keys",
                    "Order([Geo].Members, [Geo].CurrentMember.Name, ASC, [Measures].[Amount], DESC)")
            ...
            .edgeCaseMdx("symbol reserved by another function",
                    "Order([Geo].Members, [Geo].CurrentMember.Name, ALL)")

            .value("Count(Order([Geo].Members, [Geo].CurrentMember.Name))", "8")
            .valueKnownDefect("Order([Geo].Members, [Geo].CurrentMember.Name).Item(0).Name", "All Gender",
            ...
            .valueKnownDefect("Order([Geo].Members, [Geo].CurrentMember.Name, ASC).Item(0).Name", "All Gender",
                            "a method call that takes an argument does not resolve: the"
                            + " argument is lost and the receiver is typed as a numeric"
                            + " expression. The no-argument forms work. The defect is in the MDX"
                            + " parser, org.eclipse.daanse.mdx, not here")
            .valueKnownDefect("Order([Geo].Members, [Geo].CurrentMember.Name, DESC).Item(0).Name", "All Gender",
                            "a method call that takes an argument does not resolve: the"
                            + " argument is lost and the receiver is typed as a numeric"
                            + " expression. The no-argument forms work. The defect is in the MDX"
                            + " parser, org.eclipse.daanse.mdx, not here")

            .dependsOn("Order([Geo].Members, [Geo].CurrentMember.Name)")
            .doesNotDependOn("Order([Geo].Members, [Measures].[Amount], DESC)",
                       "[Geo].[Region]", "[Measures]")

            .resultStyle("Order([Geo].Members, [Geo].CurrentMember.Name)",
                         ResultStyle.MUTABLE_LIST, ResultStyle.MUTABLE_LIST)
            .resultStyle("Order([Geo].Members, [Geo].CurrentMember.Name)",
                         ResultStyle.ITERABLE, ResultStyle.MUTABLE_LIST)
            .independentMutableList("Order([Geo].Members, [Geo].CurrentMember.Name)")
