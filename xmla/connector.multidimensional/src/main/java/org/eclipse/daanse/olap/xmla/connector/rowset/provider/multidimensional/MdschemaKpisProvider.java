...
/**
 * {@code MDSCHEMA_KPIS}, as a whiteboard service.
 * <p>
 * The key performance indicators of a cube: value, goal, status, trend and
 * weight, each as the MDX that computes it.
 * <p>
...
@Component(service = RowsetProvider.class, scope = ServiceScope.PROTOTYPE, property = RowsetProvider.PROPERTY_REQUEST_TYPE
        + "=MDSCHEMA_KPIS")
public class MdschemaKpisProvider implements RowsetProvider<ContextListSupplyer> {

    @Override
    public List<EObject> rows(RowsetScope<ContextListSupplyer> scope) {
        return new MdSchemaDiscover(scope.backend(), List.of()).kpis(scope.restrictions(), scope.caller());
    }
}
