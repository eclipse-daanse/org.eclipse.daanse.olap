...
/**
 * {@code DISCOVER_CSDL_METADATA}, as a whiteboard service.
 * <p>
 * The metadata model as a CSDL-BI document, which is what reporting clients
 * read instead of the schema rowsets.
 * <p>
...
public class DiscoverCsdlMetadataProvider implements RowsetProvider<ContextListSupplyer> {

    @Override
    public List<EObject> rows(RowsetScope<ContextListSupplyer> scope) {
        return new OtherDiscover(scope.backend()).csdlMetaData(scope.restrictions(), scope.caller());
    }
}
