...
/**
 * {@code DBSCHEMA_SCHEMATA}, as a whiteboard service.
 * <p>
 * The catalogs again, this time as SQL schemata, which is where an OLE DB
...
@Component(service = RowsetProvider.class, scope = ServiceScope.PROTOTYPE, property = RowsetProvider.PROPERTY_REQUEST_TYPE
        + "=DBSCHEMA_SCHEMATA")
public class DbschemaSchemataProvider implements RowsetProvider<ContextListSupplyer> {

    @Override
    public List<EObject> rows(RowsetScope<ContextListSupplyer> scope) {
        return new DbSchemaDiscover(scope.backend()).schemata(scope.restrictions(), scope.caller());
    }
}
