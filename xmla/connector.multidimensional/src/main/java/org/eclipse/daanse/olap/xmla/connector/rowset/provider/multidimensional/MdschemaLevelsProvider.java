...
/**
 * {@code MDSCHEMA_LEVELS}, as a whiteboard service.
 * <p>
 * The levels of each hierarchy, their depth, cardinality and type.
 * <p>
...
@Component(service = RowsetProvider.class, scope = ServiceScope.PROTOTYPE, property = RowsetProvider.PROPERTY_REQUEST_TYPE
        + "=MDSCHEMA_LEVELS")
public class MdschemaLevelsProvider implements RowsetProvider<ContextListSupplyer> {

    @Override
    public List<EObject> rows(RowsetScope<ContextListSupplyer> scope) {
        return new MdSchemaDiscover(scope.backend(), List.of()).levels(scope.restrictions(), scope.caller());
    }
}
