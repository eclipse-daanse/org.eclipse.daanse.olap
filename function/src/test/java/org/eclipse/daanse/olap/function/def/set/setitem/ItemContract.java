...
public final class ItemContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("Item")
            ...
            .autoEdgeCases()
            .edgeCaseMdx("index in range",           "[Geo].Members.Item(0)")
            .edgeCaseMdx("index out of range",        "[Geo].Members.Item(100)")
            .edgeCaseMdx("index negative",            "[Geo].Members.Item(-1)")
            ...
            .waive(FunctionContract.Promise.RESULT,
                    "expected values were computed against a different catalogue and are not"
                            + " recomputed for [Test] yet; the result promise also needs a Result"
                            + " implementation, which is not lifted")
