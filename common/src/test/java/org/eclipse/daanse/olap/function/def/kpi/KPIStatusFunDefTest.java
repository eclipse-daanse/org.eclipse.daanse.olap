...
class KPIStatusFunDefTest {

    private KPIStatusFunDef funDef;

    @BeforeEach
    void setUp() {
        funDef = new KPIStatusFunDef();
    }

    @Test
    @DisplayName("Should have function name KPIStatus")
    void shouldHaveCorrectFunctionName() {
        assertThat(funDef.getFunctionMetaData().operationAtom().name()).isEqualTo("KPIStatus");
    }
    ...
    @Test
    @DisplayName("Should compile call to KPIStatusCalc")
    void shouldCompileToKPIStatusCalc() {
        ...
        assertThat(result).isInstanceOf(KPIStatusCalc.class);
    }
}
