...
public class KPICurrentTimeMemberCalc  extends AbstractProfilingNestedMemberCalc {

    protected KPICurrentTimeMemberCalc(Type type, StringCalc kpiCalc) {
        super(type, kpiCalc);
    }
    ...
    protected Member evaluateInternal(Evaluator evaluator) {
        ...
        if (oKpi.isPresent()) {
            String currentTimeMember = oKpi.get().getCurrentTimeMember();
            Member member = FunUtil.parseMember(evaluator, currentTimeMember, null);
            return member;
        } else {
            throw FunUtil.newEvalException(
                    new OlapRuntimeException("Current Time Member is absent for " + kpi));
        }
    }
