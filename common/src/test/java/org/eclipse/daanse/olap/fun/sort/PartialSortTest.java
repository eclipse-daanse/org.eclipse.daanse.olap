 ...
 * {@link Sorter#partialSort}, which supports MDX functions like TopCount and
...
class PartialSortTest {
    public static final Logger LOGGER = LoggerFactory.getLogger(PartialSortTest.class);
    ...
    private static <T extends Comparable> boolean isPartiallySorted(T[] vec, int limit, boolean descending) {
        ...
        return isPartiallySorted(vec, limit, Comparator.naturalOrder(), descending);
    }
    ...
    private Item[] doStablePartialSort(Item[] vec, boolean desc, int limit) {
        ...
        List<Item> sorted = Sorter.stablePartialSort(Arrays.asList(vec), comp, limit);
