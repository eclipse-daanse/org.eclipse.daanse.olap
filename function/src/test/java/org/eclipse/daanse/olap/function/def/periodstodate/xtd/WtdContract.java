...
public final class WtdContract {
    ...
    public static final FunctionContract CONTRACT = FunctionContract.of("Wtd")
            ...
            .dependsOnKnownDefect("Wtd()",
                            "Wtd looks for a level of type TIME_WEEKS, and the engine's"
                            + " level-type switch has no case for that value, so it fails with an"
                            + " internal error rather than saying the catalogue has no weekly"
                            + " level", "[Time]")
