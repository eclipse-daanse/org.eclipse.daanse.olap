...
class KPITrendFunDefTest {

    private KPITrendFunDef funDef;

    @BeforeEach
    void setUp() {
        funDef = new KPITrendFunDef();
    }

    @Test
    @DisplayName("Should have function name KPITrend")
    void shouldHaveCorrectFunctionName() {
        assertThat(funDef.getFunctionMetaData().operationAtom().name()).isEqualTo("KPITrend");
    }
    ...
    @Test
    @DisplayName("Should compile call to KPITrendCalc")
    void shouldCompileToKPITrendCalc() {
        ...
        assertThat(result).isInstanceOf(KPITrendCalc.class);
    }
}
